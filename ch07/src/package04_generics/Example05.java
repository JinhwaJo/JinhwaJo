...
class PacificOcean {	// 태평양 클래스
	// 아래와 같이 박스를 사용하는 방법도 익혀둘 것.
	Box<? extends Asia> box = new Box<Asia>();	// 박스 필드
...
//		return box.toString() + " 태평양"
//	}
	// ? : 와일드 카드(어떠한 파라미터 타입이 올 지 예측할 수 없기 때문에 ?를 쓴다.)
	// <? super Asia> : 파라미터 타입이 Asia 또는 Asia 조상 클래스만 올 수 있다.
	// <? extends Asia> : 파라미터 타입이 Asia 또는 Asia 자손 클래스만 올 수 있다.
...
public class Example05 {
	public static void main(String[] args) {
		...
		kbox.add(new Korea());
		System.out.println(po.toString(kbox));	// 아시아의 자손이므로 가능
		
		Box<America> ambox = new Box<America>();
		ambox.add(new America());
		// System.out.println(po.toString(ambox)); // 아시아의 자손이 아니므로 불가능
	}
}
